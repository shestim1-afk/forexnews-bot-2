...
async def run_cost_adjusted_report(api_symbol: str, spread_pct: float | None = None, strategy_types: list[str] | None = None):
    """strategy_types defaults to the 4 original detection engines
    (trend/range/liquidity_sweep/breakout_retest) for backward
    compatibility. Pass a custom list (e.g. ['vol_compression']) to
    check a newer, differently-tagged strategy instead."""
    strategy_types = strategy_types or ["trend", "range", "liquidity_sweep", "breakout_retest"]
