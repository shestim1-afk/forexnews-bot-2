...
async def run_cycle(skip_classification: bool = False):
    ...
    for item in new_items:
        ...
        # Persist every meaningful impact (not just alert-worthy ones) so
        # other tools, like the BTC scalp bot, can factor recent news
        # sentiment into their own scoring even for sub-threshold items.
        for impact in result.get("impacts", []):
            if impact.get("confidence", 0) >= 0.3:
