...
DEV_START, DEV_END = "2025-01-01", "2025-08-31"
...
async def run(period: str = "dev"):
    ...
    logger.info("Fetching 1H, 4H, and 5m BTC data for %s period...", period)
    ...
    logger.info("1H: %d candles, 4H: %d candles, 5m: %d candles", len(df_1h), len(df_4h), len(df_5m))
    ...
    days_span = max((target_end - target_start).total_seconds() / 86400, 1e-9)
    ...
    lines = [
        f"*⚡ BTC Large-Displacement Momentum ({period.upper()}): {start_date} to {end_date}*",
    ...
    if gross_avg_r is not None:
        lines.append(f"Gross expectancy: {gross_avg_r:+.3f}R/trade")
        lines.append(f"Net expectancy (base {BASE_SPREAD_PCT}% cost): {net_avg_r_base:+.3f}R/trade" if net_avg_r_base is not None else "Net expectancy: N/A")
