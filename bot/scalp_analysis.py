"""Hourly BTC/USDT scalp signal generator, using free Binance public market
data (no account or key needed for read-only candles) and free open-source
...
def fetch_klines(interval: str) -> pd.DataFrame | None:
    try:
        ...
        logger.warning("Failed to fetch %s klines: %s", interval, e)
        return None
...
def format_message(tf_data: dict, decision: dict, levels: dict, news_direction: str, news_summary: str) -> str:
    lines = [f"*{SYMBOL[:3]}/{SYMBOL[3:]}* — hourly scan\n"]
