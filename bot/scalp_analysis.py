...
def score_and_decide(tf_data: dict, news_direction: str) -> dict:
    ...
    confidence = max(0, min(100, round(score / 10 * 100)))
...
def format_symbol_block(display_symbol: str, tf_data: dict, decision: dict, levels: dict,
                         news_direction: str, news_summary: str, range_setup: dict | None) -> str:
...
def analyze_symbol(symbol_cfg: dict) -> tuple[str, dict, dict | None]:
    ...
    signal_log_data, range_setup_or_None). Used by both the hourly
    multi-symbol digest and the on-demand Telegram commands."""
    ...
    block = format_symbol_block(display, tf_data, decision, levels, news_direction, news_summary, range_setup)
