...
def fetch_filing_header_detail(cik_in_url: str, accession: str) -> dict | None:
    ...
    # FIX: the previous version let each regex search freely across the
    # ENTIRE document with DOTALL -- when a section's CIK line was
    # missing or differently formatted, the non-greedy match silently
    # crossed into the NEXT section and grabbed an unrelated CIK instead
    # of failing cleanly. Confirmed by a test case where a malformed
    # SUBJECT COMPANY section caused subject_cik to silently capture
    # FILED BY's own CIK. Now each section is explicitly bounded so a
    # match can never cross into the next labeled section.
    def extract_bounded(section_label: str, next_labels: list[str]) -> tuple[str | None, str | None]:
        start_idx = content.find(section_label)
        if start_idx < 0:
            return None, None
        end_idx = len(content)
        for nl in next_labels:
            nl_idx = content.find(nl, start_idx + len(section_label))
            if nl_idx >= 0:
                end_idx = min(end_idx, nl_idx)
        section_text = content[start_idx:end_idx]
        name_match = re.search(r"COMPANY CONFORMED NAME:\s*([^\n]+)", section_text)
        cik_match = re.search(r"CENTRAL INDEX KEY:\s*(\d+)", section_text)
        return (name_match.group(1).strip() if name_match else None,
                cik_match.group(1).strip() if cik_match else None)

    subject_name, subject_cik = extract_bounded("SUBJECT COMPANY:", ["FILED BY:", "REPORTING-OWNER:"])
    filed_by_name, filed_by_cik = extract_bounded("FILED BY:", ["REPORTING-OWNER:", "SUBJECT COMPANY:"])
    ...
    if not subject_cik:
        idx = content.find("SUBJECT COMPANY:")
        result["subject_section_raw"] = content[idx:idx + 400] if idx >= 0 else "SUBJECT COMPANY: not found in document at all"
