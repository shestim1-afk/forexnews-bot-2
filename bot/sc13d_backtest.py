...
from .stake_disclosure_probe import check_daily_index, parse_daily_index_line, fetch_filing_header_detail, SEC_USER_AGENT
...
async def run_period(period: str = "dev"):
    ...
    for line, filing_date in all_13d_lines:
        ...
        time.sleep(SEC_REQUEST_DELAY_SECONDS)
        n_processed += 1
        if not detail or not detail.get("filed_by"):
            continue
    ...
    lines.append(f"Filings processed: {n_processed}, individual-filer signals identified: {n_individual}")
